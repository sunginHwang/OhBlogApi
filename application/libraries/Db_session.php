...
//		$this->session_id = $_COOKIE[$this->cookie_name];
    ...
    public function on_session_read($key) {
        ...
        if(isset($_GET['session_key_special']) && strlen($_GET['session_key_special']) > 0)
        {
            ...
            setcookie($this->cookie_name, $this->session_id, time()+$this->expiration, '/', '.qfun.kr');
        }
    ...
    public function on_session_write($key, $val) {
        $val = session_encode();
        $expiration = $this->expiration + time();
        $insert_stmt  = "replace into ".$this->db_table_name."`(`session_id`,`ip_address`,`session_expiration`,`user_data`) values('$key',";
    ...
    public function on_session_destroy($key) {
        $sql = "delete from `$this->db_table_name` where session_id = '$key'";
        $res= mysqli_query($this->_sess_db,$sql);
    }
    public function on_session_gc($max_lifetime){
        mysqli_query($this->_sess_db,"delete from `$this->db_table_name` where `session_expiration` <unix_timestamp(now())");
    }
}
